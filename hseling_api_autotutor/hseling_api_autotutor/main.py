...
from hseling_api_autotutor import boilerplate

from hseling_lib_autotutor.process import process_data
from hseling_lib_autotutor.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
app.config.from_envvar('HSELING_API_AUTOTUTOR_SETTINGS')
app.config.update(
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_autotutor.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "Application Autotutor"})
