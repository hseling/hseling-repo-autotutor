...
from .knn_sklearn_recommendation import get_recommended_text_json, get_passed_test_count
...
def send_test_results(answers_json):
    generate_user_knowledge_database(answers_json)

def get_reading_recommendation(user_id):
    recommendations = get_recommended_text_json(user_id)
    return recommendations

def is_enough_tests_for_recommendation(username):
    passed_tests_count = get_passed_test_count(username)
    print("passed_tests_count", passed_tests_count)
    if passed_tests_count is None or passed_tests_count <= 4:
        return "not_enough_tests_passed"
    else:
        return "can_provide_recommendations"

# recommendation_algo(dic)
